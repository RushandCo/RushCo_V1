...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "Rush Token";
    string internal constant SYMBOL = "RUSH";
     ...
     * 
     * The initial supply of 1,000,000,000,000 means at launch, the maximum a wallet can hold is 1,000,000,000,000/800= 1,250,000,000. (0.125%)
     * After the 50% presale ending burn, total supply will be 500,000,000,000, meaning the maximum transaction amount will be 0.25% circulating supply.
     * This will increase as time progresses and tokens are burned.
     */
    uint256 internal constant maxTransactionAmount = TOTAL_SUPPLY / 800; 
    
    /**
     * @dev Set the maximum allowed balance in a wallet.
     * 
     *At Launch, 1,000,000,000,000/400 means each wallet will be able to hold 0.25% of the total supply. 
     *After the Presale burn, This number becomes 0.5% of the total supply
     * 
     */
    uint256 internal constant maxWalletBalance = TOTAL_SUPPLY / 400;
    ...
    address internal charityAddress = 0x7d3f9EcacaAF80aC2d1b706711a0e5362721C6c7;
    address internal burnAddress = 0x000000000000000000000000000000000000dEaD;
    address internal TreasuryAddress = 0x14685CcDdFAF1eF362a714391C3F76a10F097EF5;
    
    enum FeeType { Antiwhale, Burn, Liquidity, Rfi, External, ExternalToBNB }
    struct Fee {
        FeeType name;
        uint256 value;
        address recipient;
 ...
 * Tokenomics:
 * 
 * Liquidity Pool   (Pancake swap)      2%
 * Redistribution   (Holders)           2%
 * Burn             (continuous)        2%
 * Charity                              2%
 */
...
abstract contract BaseRfiToken is IERC20, IERC20Metadata, Ownable, Presaleable, Tokenomics {
     ...
     * 2) Tokens in the burn address cannot be sold or transfered
     *
     *
     * In RFI holders already get % of each transaction so the value of their tokens 
     * increases. Therefore there is really no need to do a "hard" burn 
     * (reduce the total supply). What matters is to make sure that a large
    ...
    function _isUnlimitedRecipient(address account) internal view returns(bool){
        ...
        // he/she wants
        return (account == owner() || account == burnAddress || account == TreasuryAddress);
    }
...
abstract contract Liquifier is Ownable, Manageable {
    ...
    event LiquidityAdded(uint256 tokenAmountSent, uint256 bnbAmountSent, uint256 liquidity);
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private {
        ...
        // add the liquidity
        (uint256 tokenAmountSent, uint256 bnbAmountSent, uint256 liquidity) = _router.addLiquidityETH{value: bnbAmount}(
            address(this),
            tokenAmount,
    ...
    */
     ...
     * 
     * The swapAndLiquify function converts half of the contractTokenBalance Rush tokens to BNB. 
     * For every swapAndLiquify function call, a small amount of BNB remains in the contract. 
     * This amount grows over time with the swapAndLiquify function being called 
     * throughout the life of the contract. 
     */
    function withdrawLockedBnb(address payable recipient) external onlyManager(){
...
abstract contract RushToken is BaseRfiToken, Liquifier, Antiwhale {
    ...
    function _takeTransactionFees(uint256 amount, uint256 currentRate) internal override {
        ...
        for (uint256 index = 0; index < feesCount; index++ ){
            ...
            if ( name == FeeType.Rfi ){
                ...
                _takeFeeToBNB( amount, currentRate, value, recipient, index );
            }
            else {
    ...
    /**
     * @dev When implemented this will convert the fee amount of tokens into BNB
     ...
     */
    function _takeFeeToBNB(uint256 amount, uint256 currentRate, uint256 fee, address recipient, uint256 index) private {
...
contract Rush is RushToken{

    constructor() RushToken(Env.MainnetV2){
